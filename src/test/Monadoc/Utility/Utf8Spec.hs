...
spec = do
  ...
  Hspec.describe "fromString" $ do
    ...
    Hspec.it "encodes UTF-8" $ do
      ...
      Utf8.fromString "\x20ac" `Hspec.shouldBe` ByteString.pack [0xe2, 0x82, 0xac]
      Utf8.fromString "\x10348" `Hspec.shouldBe` ByteString.pack [0xf0, 0x90, 0x8d, 0x88]
  ...
  Hspec.describe "toString "$ do
    ...
    Hspec.it "decodes UTF-8" $ do
      ...
      Utf8.toString (ByteString.pack [0xe2, 0x82, 0xac]) `Hspec.shouldBe` "\x20ac"
      Utf8.toString (ByteString.pack [0xf0, 0x90, 0x8d, 0x88]) `Hspec.shouldBe` "\x10348"
